...
public class LoginAspect {
    ...
    public LoginAspect(@Qualifier("redisTemplate") RedisTemplate redisTemplate) {
        this.redisTemplate = redisTemplate;
    }

    //只要有TingShuLogin注解,就进行切面逻辑
    ...
    public void process(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        //拿到注解
        TingShuLogin tingShuLogin = method.getAnnotation(TingShuLogin.class);
        ...
        if (!StringUtils.isEmpty(token)) {
            UserInfo userInfo = (UserInfo) redisTemplate.opsForValue().get(RedisConstant.USER_LOGIN_KEY_PREFIX + token);
            if (userInfo != null) {//如果redis里面有登录信息
                //这是一个线程独享的一个区域
